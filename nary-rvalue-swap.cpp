...
constexpr bool test_n(Ts &...xs)
{
  if constexpr (N==sizeof...(xs)) {
    ...
    for (std::size_t i = 0; i < N; i++) { swap(xs...); }
    std::tuple after  = std::make_tuple(xs...);
    bool left  = (before==after);
    for (std::size_t i = 0; i < N; i++) { swapr(xs...); }
...
int main(int argc, char *argv[])
{
  ...
  swapr(v1,v2,v3);
  assert(v1==v1_orig && v2==v2_orig && v3==v3_orig);
